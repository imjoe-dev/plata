...
- **Comments earn their place by explaining _why_, never _what_.** The code already says what it does; a comment that restates it is noise that goes stale. Write one only when a reader who understands the code would still ask "why like this?" — a constraint from an ADR or the domain (`Minted client-side (ADR-0002) and marked already-loaded so the hydration query skips it`), load-bearing ordering (`Claimed first, or the bridge's re-scan answers this same call a second time`), a deliberate non-obvious omission (why state is _not_ cleared), or a `// eslint-disable` justification. Specifically don't write:
  - **Narration** — `// Set the session id`, `// Loop over the messages`, or a docblock that paraphrases the signature.
  - **Archaeology** — references to code that no longer exists (`// which the landing route used to handle`, `// unlike the old banner`). The diff and the commit message carry that; the file shouldn't. Same for `// NEW:`, `// CHANGED:`, or commented-out code — git already remembers.
  - **Redundant docblocks on self-describing names** — `/** Chat Messages where Session Approval was granted. */` above `approvedMessageIds` adds nothing. If a name needs a comment to be understood, rename it first and see if the comment is still needed.

  When trimming, cut length before cutting substance: prefer one dense sentence over four hedging ones. A long comment is fine when the reasoning genuinely is long (see the History-freshness effect in `src/contexts/chat-context.tsx`).
